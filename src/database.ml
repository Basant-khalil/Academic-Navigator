...
let enrolled_group n course = List.nth course.enrolledGroups n
let group_section g = List.nth g.sections 0
      ...
      if List.length t >= 39 then
        ...
        let new_acc =
          List.fold_left
            ...
            acc lectures
        in
        course_timing t timing new_acc
      else acc
...
let credits_from_roster req_course : int =
  let major_json_list =
    roster_json |> member "data" |> member "classes" |> to_list
  in
