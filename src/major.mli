...
(** The type [libcourses] represnts a single liberal studies courses. *)
...
(** The type [major] represents the attributes of a major. *)

exception Empty
(** Raised when an empty input is given. *)

exception Malformed
(** Raised when a given input is malformed. *)

exception WrongMajor
(** Raised to represent an incorrect major. *)
...
val program_exists : string -> string -> bool
...
(** [doub first second college] prints out information about two majors
    that are contained in the same college. *)

val check_dobmajor_coll : string -> string -> string -> bool
(** [check_dobmajor_coll first second college] checks that the two given
    majors exist in the inputted college. *)
...
val return_string_course : libcourses -> string
(** [return_string_course cour] returns a string with information on
    that course. The string includes information on the name of the
    course, its category and college. Requires: [cour] is listed in one
    of the liberal studies courses. Example: return_string_course "ANSC
    4140" returns cour ANSC 4140 is in category KCM and is in college
    CALS.*)

val printdescr : lib list -> string
(** [printdescr lst] returns the description listed for that lst.
    Example: printdescr liberal_studies returns "You need at least 18
...
val matchj : engr list -> string list
(** [matchj lst] returns the info stored in the exmaple entry for fields
    like engri.*)
...
val majors_from_json : Yojson.Basic.t list -> string -> string -> bool
(** [majors_from_json lst major college] takes a major and college and
...
val majors_from_json_state :
  Yojson.Basic.t list -> string -> string -> major
(** [majors_from_json_state lst major college] takes a major and college
    and checks if that major and college are valid which happens if they
    are listed in the list passed in. If not, it returns a message to
    the user and raises an exception to show that. Else, it returns a
    record containing the infomation for that certain major.*)
...
(** [from_json major college] takes a major and college. If that major
    ...
    for that major by calling majors_from_json_state.*)
...
val doubrecord : string -> string -> string -> major
(** [doubrecord firstmaj secondmaj reqcollege] finds [firstmaj] offered
